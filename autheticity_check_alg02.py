...
import string
def extract_text_from_json(data, results=None, coordiantes = None):
    if results is None:
        results = []
        coordiantes = []
    ...
    if isinstance(data, dict):
        if 'text' in data:
            ...
            for value in data.values():
                extract_text_from_json(value, results, coordiantes)
        
        if 'polygon' in data:
            # If the key 'text' exists in the dictionary, extract its value
            coordiantes.append(data['polygon'])
        else:
            # Continue traversing nested dictionaries
            for value in data.values():
                extract_text_from_json(value, results, coordiantes)
        ...
        for item in data:
            extract_text_from_json(item, results, coordiantes)
    return results, coordiantes
...
def authenticity_score(recoverd_image_path,decripted_orginial_file_path):
    if os.path.exists(recoverd_image_path):
        ...
        text_values_recovered, cord_value_recovered = extract_text_from_json(json_data_recovered)
        print('Recovered')
        print(text_values_recovered)
        json_data_original = read_json_file(decripted_orginial_file_path)
        text_values_original, cord_value_original = extract_text_from_json(json_data_original)
        print('Recovered Cords')
        print(cord_value_recovered)
