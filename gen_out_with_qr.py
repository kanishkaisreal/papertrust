...
def add_qr_code_with_margin(input_image_path, qr_image_path, output_image_path, margin=10, margin_color="black"):
    """
    Adds a QR code at the bottom of an image with a visual separation (margin).
    ...
    total_height = main_image.height + qr_image.height + 3*margin

    # Create a blank canvas for the final image
    final_image = Image.new("RGB", (main_image.width, total_height), "white")
    ...
    draw.rectangle([margin, margin, (main_image.width-margin), (margin_top -margin)],outline=margin_color, width=4)
    draw.rectangle([margin, margin_top, (main_image.width-margin),(total_height-margin)],outline=margin_color, width=4)

    # Paste the QR code below the margin
    qr_x = (main_image.width - qr_image.width) // 2
    qr_y = margin_top + margin
    final_image.paste(qr_image, (qr_x, qr_y))

    # Save the final image
    final_image.save(output_image_path)
    print(f"Image with QR code saved to {output_image_path}.")

# Call to generate output file.
if __name__ == "__main__":
    ocr_file_path=r'F:\papertrust\papertrust\results\surya\pdf_to_ocr\pdf_to_ocr_0_text.png'
